...
type SetLogRepository interface {
	...
	GetSetLogsByWorkoutExerciseIDs(workoutExerciseIDs []uint) ([]*entity.SetLog, error)
}
...
// Batch methods for DataLoader
func (r *setLogRepository) GetSetLogsByWorkoutExerciseIDs(workoutExerciseIDs []uint) ([]*entity.SetLog, error) {
	if len(workoutExerciseIDs) == 0 {
		return []*entity.SetLog{}, nil
	}

	var setLogs []entity.SetLog
	if err := r.db.Where("workout_exercise_id IN ?", workoutExerciseIDs).Find(&setLogs).Error; err != nil {
		return nil, fmt.Errorf("failed to fetch set logs by workout exercise IDs: %w", err)
	}
